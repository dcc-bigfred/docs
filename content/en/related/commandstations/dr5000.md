...
### 8.1b Alternative: DR5000 USB LocoNet COM

The central's own USB exposes a **LocoNet** virtual COM (FTDI; JMRI:
LocoBuffer-USB). Hub kernel includes `ftdi_sio`; use after identifying the
LocoNet COM (not XpressNet / config) in DR5000 Tool:

| Catalogue field | Value |
|-----------------|-------|
| **Kind** | `loconet_serial` |
| **Connection URI** | `serial:///dev/ttyUSBn:57600` (or 115200 if Tool/JMRI so configured) |

Do **not** run DR5000 Tool and `dcc-bus` on the same LocoNet COM simultaneously.
Optional udev stub: `99-loconet-centrals.rules` → fill VID/PID/`bInterfaceNumber`
for `/dev/loconet-dr5000` after `udevadm info` on hardware.

URI parser: [`pkgs/bigfred/dcc-bus/service/station/driver.go`](../../../../pkgs/bigfred/dcc-bus/service/station/driver.go).

Example daemon invocation (63120 path):
